...
def initalizeCentroids(dataset, k):
  choices = np.random.choice(dataset.shape[0], size=k)
  centroids = dataset[choices]
  return centroids
...
def computeAssignments(dataset, centroids):
  dists = np.zeros((dataset.shape[0], centroids.shape[0]))
  for i, c in enumerate(centroids):
    norm = np.linalg.norm(dataset - c, axis=1)
    dists[:,i] = np.square(norm)
...
def updateCentroids(dataset, centroids, assignments):
  counts = np.array([0 for x in range(len(centroids))])
  for c_j in range(len(centroids)):
    points = dataset[assignments == c_j]
    centroids[c_j] = np.mean(points, axis = 0)

    counts[c_j] = points.shape[0]
...
def calculateSSE(dataset, centroids, assignments):
  for i, c in enumerate(centroids):
    norm = np.linalg.norm(dataset[assignments == i] - c, axis=1)
    sse = np.square(np.sum(norm))
  return sse
